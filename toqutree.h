...
private:
   ...
   int numberOfNodes; // number of nodes in the current toqutree
   ...
   * @param: int k, the dimension of the image (note that this is redundant
   *         --could just be grabbed from the image itself)
   * The function returns a pointer to the root of the tree that represents
   * the input PNG.
   */
   Node * buildTree(PNG * im, int k);

   /**
    * Helper function for deciding split point in toqutree.
    * Averages entropy from SE, SW, NE, NW.
    * Calls calcEntropy on each of four quadrants
    */
   double avgEntropy(PNG * imIn, int k, int x, int y, stats * stats);

   /**
    * Helper function for avgEntropy.
    * Calculates total entropy of current quadrant with upper left at (x, y).
    */
   double calcEntropy(PNG * imIn, int k, int x, int y, stats * stats);

   /**
    * Helper function for creating children sub-images in buildTree.
    */
   PNG* createSubImage(PNG * im, pair<int, int> ul, int k);

   /**
    * Helper function for navigating the quadtree and rendering the appropriate
    * HSLAPixel to each coordinate in render().
    */
   HSLAPixel findPixel(Node* node, int x, int y);

   /**
    * Helper function for mod that returns only positive numbers
    */
   int mod(int c, int p);

   /**
    * Helper function for prune() that can be called recursively
    */
